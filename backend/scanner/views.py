...
from .serializers import ScannerHistorySerializer
from .serializers import ScannerTriggerSerializer
from .models import (
    Host,
...
class ScannerHistoryListView(ListView):
    ...
    def get(self, request, type) :
        ...
        print("print", scanner_history)
        context = {
            'scanner_history' : scanner_history
        }
       # return render(request, self.template_name, context)
        return HttpResponse(json.dumps(scanner_history), content_type="application/json")

class ScannerHistoryListAPIView(generics.ListAPIView):
...
def full_scan_view(request, target):
    scanner = NmapScanner()
    scanner_history = scanner.perform_full_scan_and_save(target)
    # You can return scanner_history data as JSON
    return JsonResponse({'message': 'Full scan completed', 'scanner_history_id': scanner_history.id})


def quick_scan_view(request, target):
    scanner = ScapyScanner()
    scanner_history = scanner.save_quick_scan(target)
    # You can return scanner_history data as JSON
    return JsonResponse({'message': 'Quick scan completed', 'scanner_history_id': scanner_history.id})

class ScannerTriggerView(APIView):

    def post(self, request):
        serializer = ScannerTriggerSerializer(data=request.data)

        if serializer.is_valid():
            target = serializer.validated_data.get('target')

            # Now, you can use the 'target' value in your scanner logic

            # For example:
            # scanner = NmapScanner()
            # scanner_result = scanner.perform_full_scan_and_save(target=target)

            return Response({'message': 'Scanner triggered successfully'}, status=status.HTTP_200_OK)
        else:
